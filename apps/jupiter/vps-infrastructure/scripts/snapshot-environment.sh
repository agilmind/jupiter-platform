...
EOF
    ...
    case "$SERVICE" in
        "app-server")
            cat >> "$OUTPUT_DIR/docker-compose.yml" << EOF
    environment:
      - DATABASE_URL=postgresql://\${POSTGRES_USER:-postgres}:\${POSTGRES_PASSWORD:-postgres}@postgres:5432/\${POSTGRES_DB:-jupiter}
...
Este es un snapshot del entorno de trabajo para el proyecto jupiter.
