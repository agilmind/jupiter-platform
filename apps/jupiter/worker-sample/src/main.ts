...
async function startWorker() {
  ...
  while (retries > 0) {
    try {
      ...
      const checkConsumer = await channel.consume(
        responseQueue,
        (msg) => {
          if (msg) {
            console.log(
              `⚠️ ALERTA: Encontrado mensaje en cola ${responseQueue} que no ha sido procesado por app-server:`,
      ...
      setTimeout(() => {
        channel
          .cancel(checkConsumer.consumerTag)
...
async function checkQueue(channel, queueName) {
  try {
    ...
    console.log(
      `Cola ${queueName} - mensajes: ${queueInfo.messageCount}, consumidores: ${queueInfo.consumerCount}`
    );
    return queueInfo;
  } catch (error) {
