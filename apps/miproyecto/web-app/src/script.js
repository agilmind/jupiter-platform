document.addEventListener('DOMContentLoaded', function () {
  ...
  async function startCheck() {
    ...
    try {
      const url =
        urlInput.value.trim() || 'https://en.wikipedia.org/wiki/Main_Page';
  ...
  async function getCheckStatus() {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/check/${currentCheckId}`
      );
      ...
      if (response.ok) {
        ...
        if (data.flow && data.flow.length > 0) {
          const flowDetails = data.flow
            .map(
              (step) =>
                `${step.service}: ${step.status} (${new Date(
                  step.timestamp
                ).toLocaleTimeString()})`
            )
            .join(' -> ');
  ...
  function startPolling() {
    ...
    pollingInterval = setInterval(() => {
      ...
      if (attempts >= maxAttempts) {
        stopPolling();
        checkStatusElement.textContent =
          'Se alcanzó el tiempo máximo de espera. El scraper podría no estar funcionando.';
        return;
      }
