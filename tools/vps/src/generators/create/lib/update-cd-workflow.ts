    ...
    },
    {
      ...
      },
      {
         ...
         run: `|
          ...
          command -v rsync || { echo "ERROR: rsync command not found by 'command -v'!"; exit 1; }
          echo "Checking for rsync command using 'which':"
          which rsync || { echo "ERROR: rsync command not found by 'which'!"; exit 1; }
          echo "Attempting to get rsync version:"
          rsync --version || { echo "ERROR: failed to execute 'rsync --version'!"; exit 1; }
          echo "rsync seems accessible. Proceeding with original script..."
          echo "--- End Debugging ---"
